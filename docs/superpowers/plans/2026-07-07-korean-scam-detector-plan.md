...
Note (post-deployment manual testing, second pass): two more isolated-signal false positives surfaced. (1) "100억을 보내세요" ("send 100 billion") with no link/impersonation/urgency scored 의심/60 — an unrealistic-sized bare demand with no delivery mechanism was still read as suspicious. (2) A casual message just mentioning the app's own domain ("ahnsim-scan.vercel.app 이거 해보셈 재밌네") scored 의심/65, flagged because vercel.app is a free-hosting domain "commonly abused for phishing" — i.e. the model was treating a link's mere presence, and being on a free-hosting TLD specifically, as inherently suspicious regardless of whether any actual impersonation or credential-harvesting was happening. Generalized the existing keyword-overweighting calibration rule into a broader "isolated surface signal ≠ high risk" principle, and explicitly named vercel.app/netlify.app/github.io as free-hosting domains that are not inherently suspicious. Verified with real Gemini calls: case (1) dropped to 안전/10 with correct reasoning about the missing attack mechanism. Case (2) could not be re-verified in the same session — reproducing it surfaced a separate, real finding: this project's actual Gemini free-tier quota for `gemini-2.5-flash` is 20 requests/day (`GenerateRequestsPerDayPerProjectPerModel-FreeTier`), not the ~1,500/day this plan's Task 7 quota guard was calibrated against, and today's real quota was exhausted by cumulative testing across this session. `DAILY_LIMIT` in `quotaGuard.ts` is currently far above the real Google-side ceiling and needs to be revisited — flagged to the user rather than changed unilaterally, since the right fix (lower the internal limit vs. enable billing vs. try a different model) is a product/cost decision.

Follow-up: switched `MODEL_NAME` in `geminiProvider.ts` from `gemini-2.5-flash` to `gemini-2.5-flash-lite`, which documents a materially larger free-tier daily quota (a separate per-model quota bucket from flash, confirmed working with a real call after flash's quota was exhausted). Re-verified case (2) against flash-lite with the stronger wording above: it dropped from 의심/65 to 의심/40-45 across two runs, and the "vercel.app is commonly abused for phishing" blanket claim is gone — but the model still hedges with unfounded "위장했을 가능성을 배제할 수 없다" reasoning for an unknown-sender SMS containing a link with curiosity-bait phrasing ("이거 해보셈"), despite an explicit instruction not to. This may not be purely a bug: curiosity-bait + link from an unknown sender is a real, common smishing pattern in practice, so some residual caution for an *unknown-sender* case is arguably reasonable — the original false-positive complaint was specifically about the user's own domain in a context they knew was benign, not about how the tool should treat an anonymous incoming link in general. Diminishing returns from further prompt-only iteration on this exact case; flagged to the user as a judgment call rather than pursued further in this pass.

Note (post-deployment manual testing, third pass — a real detection gap, not a false positive): while re-testing "100만원을 송금하세요" (a realistic-amount bare money demand, distinct from the earlier absurd "100억" case), found two issues. (1) The model fabricated an evidence-free red flag — "긴급성 조성(암시적)" ("implicit" urgency) — for a message containing zero urgency language at all, the same class of unfounded-inference problem already fixed for credential-harvesting-link speculation, now generalized: `SYSTEM_PROMPT` gained a standalone instruction that every redFlags entry must be grounded in text actually present in the message, explicitly banning "가능성을 배제할 수 없다"/"암시적으로"-style invented signals. (2) Added a rule that a bare money/info demand with no supporting narrative (no accident/emergency story, no impersonated identity, no real urgency phrasing) should cap at 의심 rather than 위험 even with a realistic amount — real voice-phishing/스미싱 almost always wraps the demand in a scenario. Verified: dropped from 위험/75 to 의심/45 (later 안전/10 on a repeat run), fabricated flag gone.

**Testing the "add narrative context" fix surfaced a more serious, pre-existing gap**: a message impersonating a family member ("엄마 나 사고났어... 이 계좌로 보내줘") — one of the most common and damaging real Korean voice-phishing patterns (가족 사칭 보이스피싱/메신저피싱) — scored 안전/10, with the model's own reasoning explicitly stating "it's from mom, not an impersonation." The prompt's only impersonation signal (`정부기관, 은행, 택배사 등을 사칭하는 문구`) was scoped exclusively to institutional entities and never covered personal/family-relationship impersonation, so a message that supplies exactly the narrative context the new bare-demand rule asks for — but via family impersonation rather than institutional impersonation — fell through a real gap. Added a dedicated signal bullet for 가족·지인 사칭 (family/acquaintance impersonation: claiming to be a child/parent/spouse, messaging from an unfamiliar number with a pre-emptive excuse for the number change, an accident/emergency story, and a request to wire money to a new account), explicitly noting this needs no link or institutional domain to be a strong signal on its own. Verified: the same message now scores 위험/90 with accurate, textually-grounded reasoning. Ran a full regression sweep after this change (bare demand, absurd-amount joke, own-domain mention, benign message) — all four held at their expected low scores, confirming the new signal didn't reintroduce any of the previously-fixed false positives.
...
Note (post-deployment, Groq fallback added): this abstraction's stated purpose — swap providers without touching callers — got its first real use, sooner than expected. Real-world testing found Gemini's actual free-tier quota for this project (20 req/day on `gemini-2.5-flash`) was far below the ~1,500/day this plan assumed; switching the model to `gemini-2.5-flash-lite` (Task 4) raised the ceiling, but the more durable fix was adding a second, independent free provider as a fallback rather than depending on any single provider's quota. `analyzeMessage()` now tries Gemini first and falls back to `analyzeWithGroq()` (new file, `src/lib/analysis/groqProvider.ts`, model `openai/gpt-oss-20b` — one of only two Groq models supporting *strict* structured-output mode, matching Gemini's `responseSchema` reliability guarantee) specifically when Gemini throws `ApiError` with `status === 429` — not on any other error, so real bugs (malformed responses, schema validation failures) still surface immediately rather than being masked by a silent fallback. Requires a new `GROQ_API_KEY` env var (free, no credit card, at console.groq.com), added to `.env.local.example`/README alongside the existing five.

Real-call testing surfaced a genuine bug in the process: Groq's `gpt-oss-20b` returned `riskScore: 0.92` for a verdict of "위험" — a 0-1 ratio instead of the intended 0-100 integer scale — which passed the schema's original `min(0).max(100)` check (0.92 is numerically in range) despite directly contradicting the prompt's own stated verdict/riskScore consistency requirement. Fixed at two levels: (1) the JSON schema description for `riskScore` in both providers now explicitly says "정수(integer)... 0에서 1 사이의 비율이 아님" (integer, not a 0-1 ratio); (2) `AnalysisResultSchema` (Task 2) now enforces `.int()` and cross-validates riskScore against its verdict's documented band (0-30/31-70/71-100) via `superRefine`, so any future provider that violates this consistency gets rejected by validation rather than silently reaching the client with a wrong score.
