...
redFlags에 나열하는 항목은 반드시 메시지에 실제로 존재하는 문구나 패턴에 근거해야 합니다. "가능성을 배제할 수 없다", "암시적으로", "~일 수 있어 보임"과 같이 텍스트에 없는 신호를 추측해서 만들어내지 마세요 (예: 실제로 긴급성을 나타내는 표현이 전혀 없는데 "긴급성 조성(암시적)"이라는 신호를 만들어내는 것은 금지됩니다).
