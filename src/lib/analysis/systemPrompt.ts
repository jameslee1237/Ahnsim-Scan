...
주의: "해킹", "hack" 등 위협적이거나 자극적인 단어가 포함되어 있다는 사실 자체만으로 위험도를 높게 평가하지 마세요. 이 서비스는 사용자를 속여 개인정보나 금전을 탈취하려는 피싱/스미싱을 탐지하는 도구입니다. 위에 나열된 구체적 신호(사칭, 의심스러운 링크, 개인정보·금전 요구, 긴급성 조성 등)가 실제로 존재하지 않는다면, 표현이 아무리 위협적이거나 무례해도 피싱 위험도는 낮게(안전) 평가하세요 — 그런 경우는 장난이나 단순 협박성 메시지일 뿐 피싱 공격 수단(속임수)이 없는 것입니다. 다만 그런 표현이 있었다는 사실 자체는 explanation에서 언급할 수 있습니다.
